...
export default async function ProductsPage({
  ...
  if (query) {
    whereClause.OR = [{ name: { contains: query } }, { barcode: { contains: query } }];
  }
  if (category) {
    whereClause.category = category;
  }

  const [totalProducts, products, categoriesResult] = await Promise.all([
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-8">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        ...
        <Suspense fallback={<div className="h-10 w-full sm:w-[180px] animate-pulse bg-secondary rounded-xl" />}>
